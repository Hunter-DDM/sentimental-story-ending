...
class BiDataset(torch_data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        original_src = linecache.getline(self.original_srcF, index+1).strip().split()
        original_tgt = linecache.getline(self.original_tgtF, index+1).strip().split() if not self.char else list(linecache.getline(self.original_tgtF, index + 1).strip())
        tgtEm = float(linecache.getline(self.tgtEmF, index+1))

        return src, tgt, original_src, original_tgt, tgtEm
...
def padding(data):
    src, tgt, original_src, original_tgt, tgtEm = zip(*data)
    ...
    tgtEmTensor = torch.Tensor(tgtEm)

    return src_pad, tgt_pad, \
           torch.LongTensor(src_len), torch.LongTensor(tgt_len), \
           original_src, original_tgt, tgtEmTensor
