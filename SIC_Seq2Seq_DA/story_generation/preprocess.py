...
def makeData(srcFile, tgtFile, tgtEmFile, srcDicts, tgtDicts, save_srcFile, save_tgtFile, save_tgtEmFile, lim=0):
    sizes = 0
    ...
    tgtEmF = open(tgtEmFile, encoding='utf8')
    ...
    save_tgtEmF = open(save_tgtEmFile, 'w', encoding='utf8')

    while True:
        sline = srcF.readline()
        tline = tgtF.readline()
        tEmline = tgtEmF.readline()
...
def main():
    ...
    train_tgt_emotion = opt.load_data + 'train.' + opt.tgt_suf + '.emotion'
    valid_tgt_emotion = opt.load_data + 'valid.' + opt.tgt_suf + '.emotion'
    test_tgt_emotion = opt.load_data + 'test.' + opt.tgt_suf + '.emotion'
    ...
    save_train_tgt_emotion = opt.save_data + 'train.' + opt.tgt_suf + '.emotion'
    save_valid_tgt_emotion = opt.save_data + 'valid.' + opt.tgt_suf + '.emotion'
    save_test_tgt_emotion = opt.save_data + 'test.' + opt.tgt_suf + '.emotion'
    ...
    train = makeData(train_src, train_tgt, train_tgt_emotion, dicts['src'], dicts['tgt'], save_train_src, save_train_tgt, save_train_tgt_emotion)

    print('Preparing validation ...')
    valid = makeData(valid_src, valid_tgt, valid_tgt_emotion, dicts['src'], dicts['tgt'], save_valid_src, save_valid_tgt, save_valid_tgt_emotion)

    print('Preparing test ...')
    test = makeData(test_src, test_tgt, test_tgt_emotion, dicts['src'], dicts['tgt'], save_test_src, save_test_tgt, save_test_tgt_emotion)
