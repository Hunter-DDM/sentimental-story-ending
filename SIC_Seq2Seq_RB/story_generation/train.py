...
def build_model(checkpoints, print_log):
    ...
    if checkpoints is not None:
        ...
        print('==============================================')
        print(model.decoder.sum_weight1)
        print(model.decoder.sum_weight2)
        print('==============================================')
    if opt.pretrain:
...
def train_model(model, data, optim, epoch, params):
    ...
    for src, tgt, src_len, tgt_len, original_src, original_tgt, tgtEm in trainloader:
        ...
        tgtEm = torch.index_select(tgtEm, dim=0, index=indices)
        dec = tgt[:, :-1]
        targets = tgt[:, 1:]
        ...
        try:
            if config.schesamp:
                if epoch > 8:
                    e = epoch - 8
                    loss, outputs = model(src, lengths, dec, targets, tgtEm, teacher_ratio=0.9**e)
                else:
                    loss, outputs = model(src, lengths, dec, targets, tgtEm)
            else:
                loss, outputs = model(src, lengths, dec, targets, tgtEm)
        ...
        if params['updates'] % 20 == 0:
            print (params['updates'], 'iters, loss =', float(loss))
        params['updates'] += 1
...
def eval_model(model, data, params):
    ...
    for src, tgt, src_len, tgt_len, original_src, original_tgt, tgtEm in validloader:
        ...
        with torch.no_grad():
            if config.beam_size > 1:
                samples, alignment, weight = model.beam_sample(src, src_len, tgtEm, beam_size=config.beam_size, eval_=True)
            else:
                samples, alignment = model.sample(src, src_len, tgtEm)
...
def eval_model_test700_bleu(model, data, params):

    model.eval()
    reference, candidate, source, alignments = [], [], [], []
    count, total_count = 0, len(data['testset'])
    
    testloader = torch.utils.data.DataLoader(dataset=data['testset'],
                                              batch_size=config.batch_size,
                                              shuffle=False,
                                              num_workers=0,
                                              collate_fn=utils.padding)
    ...
    for src, tgt, src_len, tgt_len, original_src, original_tgt, tgtEm in testloader:
...
def eval_model_test700_five_emo(model, data, params):
...
def main():
    ...
    if opt.mode == "train":
        ...
        if opt.mode == "eval":
            ...
            if opt.mode == "eval_test700_bleu":
                eval_model_test700_bleu(model, data, params)
            else:
                eval_model_test700_five_emo(model, data, params)
