...
class seq2seq(nn.Module):
    ...
    def forward(self, src, src_len, dec, targets, tgtEm, teacher_ratio=1.0):
        src = src.t()
        dec = dec.t()
        targets = targets.t()
        ...
        if teacher:
            for input in dec.split(1):
                output, state, attn_weights = self.decoder(input.squeeze(0), state, tgtEm)
                outputs.append(output)
            outputs = torch.stack(outputs)
        else:
            inputs = [dec.split(1)[0].squeeze(0)]
            for i, _ in enumerate(dec.split(1)):
                output, state, attn_weights = self.decoder(inputs[i], state, tgtEm)
    ...
    def sample(self, src, src_len, tgtEm):
    ...
    def beam_sample(self, src, src_len, tgtEm, beam_size=1, eval_=False):
