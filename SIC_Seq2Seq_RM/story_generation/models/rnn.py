...
class rnn_decoder(nn.Module):
    ...
    def __init__(self, config, embedding=None, use_attention=True):
        ...
        # emotion modified begin
        self.embedding_emotion = nn.Embedding(config.tgt_vocab_size, config.emb_size)
        self.W_emotion = nn.Parameter(torch.rand(config.emb_size, 1), requires_grad=True)
        self.B_emotion = nn.Parameter(torch.rand(1), requires_grad=True)
        self.sum_weight1 = nn.Parameter(torch.rand(1), requires_grad=True)
        self.sum_weight2 = nn.Parameter(torch.rand(1), requires_grad=True)
        self.sigmoid = nn.Sigmoid()
        # emotion modified end
    ...
    def forward(self, input, state, tgtEm):
        ...
        # emotion modified begin
        batch_emotion_score = tgtEm  # batch * 1
        emotion_weight = torch.mm(self.embedding_emotion.weight, self.W_emotion) + self.B_emotion  # vocab_size * 1
        emotion_weight = self.sigmoid(emotion_weight)  # vocab_size * 1
        emotion_weight = emotion_weight.t()  # 1 * vocab_size
        batch_factor = [torch.mul((emotion_score - emotion_weight), (emotion_score - emotion_weight)) / 2.0 for emotion_score in batch_emotion_score]  # batch * 1 * vocab_size
        batch_factor = torch.stack(batch_factor).squeeze()  # batch * vocab_size
        batch_gaussian_p = torch.exp(-batch_factor) * math.pow(2.0 * math.pi, -0.5)  # batch * vocab_size
        # 先stack，squeeze以避免两次列表操作，可以加速运算。
        # batch_gaussian_p = [torch.exp(-factor) * math.pow(2 * math.pi, -0.5) for factor in batch_factor]
        output = output * self.sum_weight1 + batch_gaussian_p * self.sum_weight2
        # emotion modified end
