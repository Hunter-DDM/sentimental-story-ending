...
class simple_attention(nn.Module):
    ...
    def forward(self, h):
        # (Batch_size, Length, Hidden_size) -> (Batch_size , Length, 1)
        weights = self.projection(h)
        weights = F.softmax(weights.squeeze(-1), dim=1)
        # (Batch_size, Length, Hidden_size) * (Batch_size, Length, 1) -> (Batch_size, Hidden_size)
        outputs = (h * weights.unsqueeze(-1)).sum(dim=1)
